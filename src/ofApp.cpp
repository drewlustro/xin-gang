...
#define FPS_GLUE 60
#define ENABLE_PRIMATIVES false
#define ENABLE_VIDEO false
#define ENABLE_FPS_WINDOW true
//--------------------------------------------------------------
void ofApp::setup(){
	
	ofSetFrameRate(FPS_GLUE);



	
	if (ENABLE_VIDEO) {
		video.loadMovie("vail-gpro-4k-1.mp4");
		// video.loadMovie("vail-gpro-1080p-1.mov");
		video.play();
	}

	if (ENABLE_PRIMATIVES) setupPrimatives();
}
...
void ofApp::update(){
	...
	float val = sin(time * M_TWO_PI);
	float c = ofMap(val, -1.0, 1.0, 0, 255.0, true);
	ofBackground(c, c, c);

	

	if (ENABLE_FPS_WINDOW) {
	...
	// ----
	if (ENABLE_PRIMATIVES) updatePrimatives();
}
...
void ofApp::draw(){
	...
	// ----
	if (ENABLE_PRIMATIVES) drawPrimatives();
}

//--------------------------------------------------------------
void ofApp::keyPressed(int key) {
    
	if (ENABLE_PRIMATIVES) keyPressedPrimatives(key);
...
//--------------------------------------------------------------
void ofApp::keyPressedPrimatives(int key) {
